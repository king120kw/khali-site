...
class ArcballControl {
    ...
    update(deltaTime, targetFrameDuration = 16) {
        ...
        if (this.isPointerDown) {
            const INTENSITY = 0.5 * timeScale; // Increased from 0.3 for smoother response
            const ANGLE_AMPLIFICATION = 3.5 / timeScale; // Reduced from 5 for less drag
            ...
            const INTENSITY = 0.15 * timeScale; // Increased from 0.1 for smoother settling
            quat.slerp(this.pointerRotation, this.pointerRotation, this.IDENTITY_QUAT, INTENSITY);

            if (this.snapTargetDirection) {
                const SNAPPING_INTENSITY = 0.25; // Increased from 0.2 for smoother snap
...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
    ...
    return (
        <div style={{ position: 'relative', width: '100%', height: '100%' }}>
            ...
            {activeItem && (
                <>
                    <div className={`step-indicator ${isMoving ? 'inactive' : 'active'}`}>
                        <span className="step-number">
                            {String(items.findIndex(item => item.image === activeItem.image) + 1).padStart(2, '0')}
                        </span>
                        <span className="step-divider">/</span>
                        <span className="step-total">{String(items.length).padStart(2, '0')}</span>
                    </div>
